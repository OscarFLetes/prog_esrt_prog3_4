/* Autor:Oscar Eduardo FLetes Ixta, 07/04/22
	...
	Debe tener el siguiente menu:
		...
		10. Ordenar por promedio
		
	Programa en lenguaje c, que utilizando la misma estructura y menu del prog_est_prog3_3, solo agregamos al menu las opciones 
	ordenamineto por promedio, con el uso de estructuras, prototipos de funciones, switch, uso de milibreria.h
	...
	Objetivo:
	Mostrar el uso de variables char, float y int, condiciones, uso de la libreria string, milibreria.h, stdlib.h, funciones para cadenas de caracteres, 
	uso de funciones con parametros, prototipos de funciones, ciclo while, ciclo for, realizar operaciones aritmeticas e imprimirla.

	Breve descripcion:
	El programa  pide como entrada un menu para que tiene como opciones, agregar, imprimir, alumnos aprobados, alumnos reprobados,promedio del grupo, 
	porcentaje de los alumnos aprobados, guardar archivo, cargar archivo, ordenar por nombre, ordenar por promedio,  al seleccionar la opcon 1(agregar) 
...
void ordenarProm(struct alumno x[]);//Funcion con parametros
//Principal
int main(){
	...
	do{//ciclo do-while
		...
		switch(opcion){//switch
			...
			case 9: ordenarNombre(grupo);//ejecuta la funcion ordenar por nombre con sus parametros
					break;//Es para que termine la iteraccion en ejecucucion y volver al principio, para realizar otra iteraccion
			case 10: ordenarProm(grupo);//ejecuta la funcion ordenar por promedio con sus parametros
					break;//ejecuta la funcion ordenar por nombre con sus parametros
		}
...
int menu(){
	...
	printf("------------------------------------\n");//imprime mensaje
	opcion=leerd("Selecciona una opcion: ",0,10);//valida que el menu solo acepte de 0-10
	return opcion;
}

void guardar(struct alumno x[]){//funcion guardar archivo(parametros)
FILE*f;//variable para archivos
	f=fopen("Alunos.txt","w");//abre y guarda el archivo en modo write(w)
	...
	if(f==NULL){//condicon, si f es igual a null
		...
		for(int i=0;i<n;i++){//ciclo for para acumular los datos de los alumnos insertados
			fprintf(f,"%s\t%f\t%d\n",x[i].nombre,x[i].promedio,x[i].faltas);//guarda los datos
		}
...
void cargar(struct alumno x[]){//funcion cargar archivo(parametros)
	FILE*f;//variable para archivos
	struct alumno a;//estrucutura del alumno
	f=fopen("Alunos.txt","r");//abre el archivo en modo read (r)
	...
	if(f==NULL){//condicon, si f es igual a null
		...
		printf("Leyendo......\n");//imprime mensaje
		//ciclo while
		while(fscanf(f,"%30[^\t]%f\t%d\n",a.nombre,&a.promedio,&a.faltas)!=EOF){//leer los resgitros hasta llegar al utimo caracter
			//EOF caracter final del archivo
			x[n]=a;//x en el arreglo n es igual a la estructura del alumno a
			n++;//cuenta a los alumnos
			printf("%3d\t%-30s\t%4.1f\t%2d\n",n,a.nombre,a.promedio,a.faltas);//imprime eel numero del alumno, el nombre, promedio y faltas 
		}
...
void ordenarProm(struct alumno x[]){//funcion ordenar por promedio(parametros)
	//Ordenamiento del arreglo por el metodo de la burbuja
	for(int z = 1; z < n; ++z) {
		struct alumno aux;//se declara una variable aux para quese utilice la estrcutura del alumno
      	for(int v = 0; v < (n - z); v++) {
